...
from .talkconv_module import TaLKConv

@register_model('talkconv')
class TaLKConvModel(FairseqEncoderDecoderModel):
    """
    Args:
        encoder (TaLKConvEncoder): the encoder
        decoder (TaLKConvDecoder): the decoder
    """
    ...
    def build_model(cls, args, task):
        ...
        encoder = TaLKConvEncoder(args, src_dict, encoder_embed_tokens)
        decoder = TaLKConvDecoder(args, tgt_dict, decoder_embed_tokens)
        return TaLKConvModel(encoder, decoder)
...
class TaLKConvEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.layers.extend([
            TaLKConvEncoderLayer(args, kernel_size=args.encoder_kernel_size_list[i])
...
class TaLKConvEncoderLayer(nn.Module):
    """Encoder layer block.
...
@register_model_architecture('talkconv', 'talkconv')
def base_architecture(args):
    ...
    args.encoder_kernel_size_list = getattr(args, 'encoder_kernel_size_list', [1, 3, 7, 15, 15, 15, 15])
    args.decoder_kernel_size_list = getattr(args, 'decoder_kernel_size_list', [1, 3, 7, 15, 15, 15])



@register_model_architecture('talkconv', 'talkconv_wmt_en_de_big')
def talkconv_wmt_en_de_big(args):
...
@register_model_architecture('talkconv', 'talkconv_wmt_en_fr_big')
def talkconv_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    talkconv_wmt_en_de_big(args)


@register_model_architecture('talkconv', 'talkconv_iwslt_de_en')
def talkconv_iwslt_de_en(args):
   args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
...
@register_model_architecture('talkconv', 'talkconv_cnndm_std')
def talkconv_cnndm_std(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 2048)
    args.encoder_attention_heads = getattr(args, 'encoder_attention_heads', 8)
    args.encoder_layers = getattr(args, 'encoder_layers', 7)
    ...
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
    args.attention_dropout = getattr(args, 'weight_dropout', 0.1)
    args.dropout = getattr(args, 'dropout', 0.3)
...
@register_model_architecture('talkconv', 'talkconv_cnndm_deep')
def talkconv_cnndm_deep(args):
    args.encoder_layers = getattr(args, 'encoder_layers', 10)
    args.decoder_layers = getattr(args, 'decoder_layers', 10)

    args.encoder_kernel_size_list = getattr(args, 'encoder_kernel_size_list', [3, 7, 15, 31, 31, 31, 31, 31, 31, 31])
    args.decoder_kernel_size_list = getattr(args, 'decoder_kernel_size_list', [3, 7, 31, 31, 31, 31, 31, 31, 31, 31])
    talkconv_cnndm_std(args)